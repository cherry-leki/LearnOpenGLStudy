...
def main(args):
    ...
    # set shader program
    obj_shader   = Shader('02_basic_lighting/02_object.vs',   '02_basic_lighting/02_object.fs')
    light_shader = Shader('02_basic_lighting/02_lighting.vs', '02_basic_lighting/02_lighting.fs')
    ...
    while not glfw.window_should_close(window):
        ...
        # Task 2: Rotating light cube
            ...
            light_position = glm.vec3(rotX, 0, rotZ)

            model_mat = glm.mat4(1.0)
            model_mat_list.append(model_mat)
        # Task 3: Test specular light
        elif args.task == 3:
            light_position = glm.vec3(1.2, 0.0, 0.0)
        ...
        # draw object
        obj_shader.use()
        obj_shader.set_vec3("objectColor", 1.0, 0.5, 0.31)
        obj_shader.set_vec3("lightColor", 1.0, 1.0, 1.0)
        obj_shader.set_vec3("lightPos", light_position)
        obj_shader.set_vec3("viewPos", camera.position)
        obj_shader.set_int("shininess", 256 if args.task == 3 else 32)
